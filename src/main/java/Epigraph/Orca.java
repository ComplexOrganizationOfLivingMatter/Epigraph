...
public class Orca {
	...
	public void countingFullGraphlets() {
		int[] C5 = new int[this.adjacencyMatrix[0].length];
	...
	public void buildingEquationSystems(){
		...
		for (int x=0;x< this.adjacencyMatrix[0].length;x++) {
			...
			for (int nx1=0;nx1<deg[x];nx1++) {
				int a=Math.min(inc[x][nx1].getA(), inc[x][nx1].getB()), xa=Math.max(inc[x][nx1].getA(), inc[x][nx1].getB());
				...
				for (int na=0;na<deg[a];na++) {
					...
					for (int nb=0;nb<deg[b];nb++) {
						int c=adj[b][nb];
						if (c==a || isAdjacent(a,c) == 1) continue;
				...
				for (int nx2=nx1+1;nx2<deg[x];nx2++) {
					int b=Math.min(inc[x][nx2].getA(), inc[x][nx2].getB()), xb=Math.max(inc[x][nx2].getA(), inc[x][nx2].getB());
					if (isAdjacent(a,b) == 0) continue;
					for (int nx3=nx2+1;nx3<deg[x];nx3++) {
						int c=Math.min(inc[x][nx3].getA(), inc[x][nx3].getB()), xc=Math.max(inc[x][nx3].getA(), inc[x][nx3].getB());
						if (isAdjacent(a,c) == 0 || isAdjacent(b,c) == 0) continue;
						orbit[x][14]++;
